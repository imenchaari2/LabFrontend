...
export class ArticleService {
    ...
    public addArticle(creatorId: string, article: Article, source: File): Observable<Article> {
        const formData = new FormData();
        for (const key in article) {
            formData.append(key, article[key]);
        }
        formData.append('file', source);
        return this.http.post<Article>(`${this.apiUrl}/addArticle/${creatorId}`, formData);
    }
